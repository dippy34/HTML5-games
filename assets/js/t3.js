...
window.addEventListener("load", () => {
  ...
  // Register Nova Hub cache service worker
  ...
  function processUrl(url) {
    ...
    const proxiedUrl = isYouTube ? `/a/q/${__uv$config.encodeUrl(url)}` : `/a/${__uv$config.encodeUrl(url)}`;
    
    activeIframe.src = proxiedUrl;
...
document.addEventListener("DOMContentLoaded", event => {
  ...
  function createNewTab() {
    ...
    // Helper function to get proxied URL with YouTube detection
    const getProxiedUrl = (encodedUrl) => {
      if (encodedUrl.includes("/e/")) {
        return window.location.origin + encodedUrl;
      }
      // Try to decode to check if it's YouTube
      try {
        const decoded = __uv$config.decodeUrl(encodedUrl);
        const isYouTube = decoded.includes('youtube.com') || decoded.includes('youtu.be');
        return isYouTube 
          ? `${window.location.origin}/a/q/${encodedUrl}`
          : `${window.location.origin}/a/${encodedUrl}`;
      } catch {
        // If decoding fails, default to Ultraviolet
        return `${window.location.origin}/a/${encodedUrl}`;
      }
    };

    if (tabCounter === 0 || tabCounter === 1) {
      if (goUrl !== null) {
        newIframe.src = getProxiedUrl(goUrl);
      } else {
        newIframe.src = "/";
      }
